...
#implement a linear SVM. What is the accuracy of your SVM for year 2?
model = SVC(kernel='linear')
model.fit(year1_features,year1_labels)
score = model.score(year1_features,year1_labels)
print('Accuracy Score of linear SVM classifier for year 1 is:',score)

pred_lst = model.predict(year2_features)
output_result(year2_weeks['label'].to_list(),pred_lst)


#implement a Gaussian SVM and compute its accuracy for year 2? Is it better than linear SVM (use default values for parameters)
model2 = SVC(kernel='rbf')
model2.fit(year1_features,year1_labels)
score2 = model2.score(year1_features,year1_labels)
print('Accuracy Score of Gaussian SVM classifier for year 1 is:',score2)

pred_lst2 = model2.predict(year2_features)
output_result(year2_weeks['label'].to_list(),pred_lst2)


#implement polynomial SVM for degree 2 and compute its accuracy? Is it better than linear SVM?
model3 = SVC(kernel='poly')
model3.fit(year1_features,year1_labels)
score3 = model3.score(year1_features,year1_labels)
print('Accuracy Score of Polynomial SVM classifier for year 1 is:',score3)

pred_lst3 = model3.predict(year2_features)
output_result(year2_weeks['label'].to_list(),pred_lst3)


#implement a trading strategy based on your labels (from linear SVM) for year 2 and compare the performance with the ”buy-and-hold” strategy. Which strategy results in a larger amount at the end of the year?
